...
public class HashMapOpenAddressing {
	...
	public int hash(int k,int lenght){
		if(this.h.equals("DivisionMethod"))
			return HashFunction.DivisionMethod(k,lenght);
		else
			return HashFunction.KnuthMethod(k,lenght);
	...
	public boolean add(int k, String v) {
		...
		while(kolizije < table.length){
			if(table[hash].equals(new Element(Integer.MIN_VALUE,v))){
				table[hash] = new Element(k,v);
				flag = true;
			}
			hash=(hash+kolizije) % table.length;
			kolizije++;
		}
		return flag;
