...
public class HashFunction {
	...
	public static int DivisionMethod(int k, int m) {
		// new UnsupportedOperationException("You need to implement this function!");
		if(k < 0)
			k*=-1;
		return k%m;
	}
	...
	public static int KnuthMethod(int k, int m) {
		...
		double goldenCut = (sqrt(5)-1) / 2; // A
		if(k < 0)
			return (int) (m*(k*-1 * goldenCut % 1));
		else
			return (int)(m*(k*goldenCut%1));
	}
}